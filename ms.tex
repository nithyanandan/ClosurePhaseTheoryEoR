...
The bi-spectrum in the context of interferometry has been investigated in \cite{jen58,kul89,tay99,tho01,mon06} and recently revisited in \cite{car18}. Consider a triad of antennas, $\{a, b, c\}$, indexed by $p$. $V_{pq}^\textrm{m}(f)$ is the spatial coherence measured at frequency, $f$, between antenna pairs, $\{ab, bc, ca\}$, indexed by $ij$. An interferometer measures $V_{pq}^\textrm{m}(f) = g_p(f)\, g_q^*(f)\, V_{pq}^\textrm{T}(f) + V_{pq}^\textrm{N}(f)$, the  true spatial coherence, $V_{pq}^\textrm{T}(f)$, corrupted by multiplicative antenna gains and ionospheric distortions, $g_p$, and additive thermal-like noise, $V_{pq}^\textrm{N}(f)$. We express $V_{pq}^\textrm{T}(f) = V_{pq}^\textrm{F}(f) + V_{pq}^\textrm{H{\sc i}}(f)$, where, $V_{pq}^\textrm{F}(f)$ and $V_{pq}^\textrm{H{\sc i}}(f)$ are due to foreground synchrotron and EoR \HI\ signal, respectively. The measured bi-spectrum is $B_\nabla^\textrm{m}=\prod_{pq} V_{pq}^\textrm{m}=B_\nabla^\textrm{T}\prod_p |g_p|^2+B_\nabla^\textrm{N}$, where, the dependence on $f$ has been omitted for convenience (hereafter so, unless indicated). In our notation, $ij$ only indexes three specific cyclic antenna pairs $\{ab, bc, ca\}$ and not all possible combinations of $p$ and $q$. $B_\nabla^\textrm{N}$, encompassing all terms containing $V_{pq}^\textrm{N}$ including cross-terms, has zero mean since it contains $V_{pq}^\textrm{N}$, which is assumed to be uncorrelated between antenna pairs. The phase of $B_\nabla^\textrm{m}$ is:
\begin{align}
  \phi_\nabla^\textrm{m} &= \sum_{pq}\phi_{pq}^\textrm{m} = \phi_\nabla^\textrm{T} + \delta\phi_\nabla^\textrm{N} = \left(\phi_\nabla^\textrm{F} + \delta\phi_\nabla^\textrm{H{\sc i}}\right) + \delta\phi_\nabla^\textrm{N} \label{eqn:cpphase-sum},
\end{align}
where, $\phi_{pq}^\textrm{m} = \phi_p - \phi_q + \phi_{pq}^\textrm{T} + \delta\phi_{pq}^\textrm{N}$, and $\phi_p$ are the phases of $V_{pq}^\textrm{m}$ and $g_p$, respectively. $\delta\phi_\nabla^\textrm{H{\sc i}}$ and $\delta\phi_\nabla^\textrm{N}$ are not just determined by the phases of $V_{pq}^\textrm{H{\sc i}}$ and $V_{pq}^\textrm{N}$ but are small  perturbations due to the EoR \HI\ signal and noise respectively, superimposed on dominant phase, $\phi_\nabla^\textrm{F}$, due to foregrounds, and depend on $\rho_{pq}^\textrm{H{\sc i}}\equiv |V_{pq}^\textrm{F}|/|V_{pq}^\textrm{H{\sc i}}|$ and $\rho_{pq}^\textrm{N}\equiv |V_{pq}^\textrm{F}|/|V_{pq}^\textrm{N}|$ respectively. The small-perturbation formalism is valid when $\rho_{pq}^\textrm{H{\sc i}}\gg 1$ and $\rho_{pq}^\textrm{N}\gg 1$, as is usually the case in low-frequency cosmology experiments. $\phi_\nabla^\textrm{m}$ is independent of the antenna gains and measures $\phi_\nabla^\textrm{T}$, corrupted only by noise, $\delta\phi_\nabla^\textrm{N}$. Hence, $\phi_\nabla^\textrm{m}$ contains information purely about the sky's spatial structure, with certain geometric properties \cite[see][]{mon07}.

$\delta\phi_{pq}^\textrm{N}$ follows a Gaussian distribution with standard deviation, $\sigma_{\phi_{pq}^\textrm{N}} = (\sqrt{2}\,\rho_{pq}^\textrm{N})^{-1}$, when $\rho_{pq}^\textrm{N}\gg 1$ \cite{cra89}. From Eq.~(\ref{eqn:cpphase-sum}), for $\rho_{pq}^\textrm{N}\gg 1$, $\phi_\nabla^\textrm{m}$ will also approach a Gaussian distribution with variance, $\sigma_{\phi_\nabla^\textrm{m}}^2 = \sum_{pq}\,(\sqrt{2}\,\rho_{pq})^{-2}$, where, $\rho_{pq}=\rho_{pq}^\textrm{N}$ (if $\rho_{pq}^\textrm{N} < \rho_{pq}^\textrm{H{\sc i}}$) or $\rho_{pq}=\rho_{pq}^\textrm{H{\sc i}}$ (if $\rho_{pq}^\textrm{H{\sc i}} < \rho_{pq}^\textrm{N}$). $\delta\phi_\nabla^\textrm{H{\sc i}}$ can be made the dominant fluctuation in $\phi_\nabla^\textrm{m}$ by averaging a number of independent samples ($N_\textrm{m}$) of $\phi_\nabla^\textrm{m}$ and reducing the variance of $\delta\phi_\nabla^\textrm{N}$ to $\sigma_{\delta\phi_\nabla^\textrm{N}}^2 / N_\textrm{m}$. 
...
Thus, when $\rho_{pq}^\textrm{H{\sc i}} < \rho_{pq}^\textrm{N}$, $\sigma_{\phi_\nabla^\textrm{m}}$ can be used to estimate $\rho_{pq}^\textrm{H{\sc i}}$, which in turn can be used to infer $|V_{pq}^\textrm{H{\sc i}}|$ if $|V_{pq}^\textrm{F}|$ is known. $|V_{pq}^\textrm{H{\sc i}}|$ is directly related to the EoR H\,{\sc i} brightness temperature fluctuations, $\langle(\delta T_\textrm{b}^\textrm{H{\sc i}})^2\rangle$, which can be inferred as a function of  redshift. This is discussed further below.
...
The primary application of this technique is to detect the EoR \HI\ signal. Using an {\it Airy} power pattern for the HERA dish, we used PRISim \footnote{The Precision Radio Interferometry Simulator (PRISim) Python package is available at \href{https://github.com/nithyanandan/PRISim}{https://github.com/nithyanandan/PRISim}.} to simulate $V_{pq}^\textrm{F}(f)$, $V_{pq}^\textrm{H{\sc i}}(f)$, and $V_{pq}^\textrm{N}(f)$ from the synchrotron foregrounds, an EoR \HI\ model, and noise, respectively as described in \S\ref{sec:modeling} over the 100--200~MHz band with 195.3125~kHz spectral resolution, and a conservative phase-coherent integration interval of 1~min \cite{car18}. 
