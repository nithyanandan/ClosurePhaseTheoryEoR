...
Fig.~\ref{fig:cp-spectra} shows the the bi-spectrum phase spectra, $\phi_\nabla$, for the foreground and EoR components separately. Clearly, the foreground component is characterized by a smoother spectrum whereas the EoR component fluctuates rapidly. The observed $\phi_\nabla^\textrm{m}$, that includes the foregrounds, EoR \HI\, and noise components, will be described by Eq.~\ref{eqn:cpphase-sum}. 
...
% Since $\Xi_\nabla(\tau)$ results from the convolution of the delay spectrum responses of $e^{i\phi_{ij}^\textrm{m}}$, 
As a result of the convolution, the foreground contribution to $\Xi_\nabla(\tau)$ is expected to be more extended along $\tau$ (and $k_\parallel$) relative to $\Xi_{ij}(\tau)$. This relation will be explored in a forthcoming study. 
...
Excision of radio frequency interference (RFI) affected frequencies, if not carefully corrected for, will introduce spectral artifacts, and reduce the sensitivity in relatively foreground-free $k_\parallel$-modes. However, it must be noted that most existing approaches face these challenges as well. In future work, we will investigate the challenges and the detectability of EoR using this approach on HERA data.
