...
We have presented the concept behind this new approach using simple analysis. There are a number of ways in which the prospects of detecting EoR can be improved.

\begin{enumerate}
\item The completed layout of HERA will consist of 350 antennas. More data.
\item Optimized analysis to get maximum sensitivity
\item One-dimensional power spectrum, improved sensitivity from binning
\end{enumerate}

We identify the following challenges for this approach.

\begin{enumerate}
\item {\it EoR window} is smaller
\item Baseline-based gain terms
\item Off-diagonal Jones matrix in an antenna
\item Non-redundancy -- antenna beams and other causes
\item Other systematics
\end{enumerate}

The above challenges are not a problem if they are spectrally smooth.

Current HERA in a season of observing will have enough data to provide constraints or clearly indicate the limitations.
