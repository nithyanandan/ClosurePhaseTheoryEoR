...
\begin{abstract}
Detecting neutral Hydrogen (H~{\sc i}) via 21~cm line emission from the intergalactic medium at $z\gtrsim 6$ has been identified as one of the most promising probes of the epoch of cosmic reionization -- a major phase transition of the Universe. However, the first detection of this emission remains unfulfilled, due to the experimental challenges imposed by the orders-of-magnitude brighter emission arising from foreground cosmic objects. Current techniques require extremely precise instrumental calibration to separate the weak H~{\sc i} line signal from the foreground continuum emission. We propose using the phase of the bi-spectrum from interferometric measurements to mitigate the requirement for such precise calibration. The closure phase carries information purely about the sky, besides stochastic measurement noise, and is unaffected by antenna-based calibrations and uncertainties therein. We show that the bi-spectrum phase of foreground continuum emission has a characteristically smooth spectrum relative to the cosmological line signal. Thus, the latter can be separated effectively by exploiting this spectral difference using a Fourier-domain approach, while eliminating the requirement for precise antenna-based calibration of phases introduced by the instrument, and the ionosphere, inherent in existing approaches. Using fiducial models for continuum foregrounds, and for the cosmological H~{\sc i} signal, we show the latter should be detectable in bi-spectrum phase spectra, with reasonable significance at $|k_\parallel| \gtrsim 0.5\,h$~Mpc$^{-1}$, using existing instruments. 
\end{abstract}
...
All noise-like terms on the R.H.S. are uncorrelated due to the presence of the noise term and average to zero.
